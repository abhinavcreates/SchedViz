  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4">
        ...
        {/* Algorithm Selector — uses Framer Motion layoutId for the sliding active indicator */}
        <div className="flex flex-wrap gap-1 border-b border-border">
          {ALGORITHMS.map(algo => (
            <button
              ...
              style={{
                color: selectedAlgo === algo.id ? '#e07a3f' : '#9a9080',
              }}
            >
              {algo.label}
              {/* Sliding underline indicator — shared layoutId makes it animate smoothly */}
              {selectedAlgo === algo.id && (
                <motion.div
                  layoutId="cpu-tab-indicator"
      ...
      <div className="grid lg:grid-cols-[350px_1fr] gap-8 mt-4">
        {/* Left Column: Form — no animation on form inputs per spec */}
        <div className="flex flex-col gap-4">
          ...
          {error && (
            <div className="p-3 bg-error/10 text-error border border-error/20 text-sm">{error}</div>
          )}
        </div>

        {/* Right Column: Results */}
        <div className="flex flex-col gap-8">
          <h2 className="text-lg font-medium text-white border-b border-border pb-2">
            Simulation Results
          </h2>
          ...
          {loading ? (
            ...
            <AnimatePresence mode="wait">
              {result ? (
                /* Results panel — fades in and slides up from 10px when results arrive */
                <motion.div
                  key={`results-${result.avgWaiting}`}
                ...
                >
                  <div className="flex flex-col gap-2">
                    ...
                    {/* Gantt chart animation is handled internally — left untouched */}
                    <GanttChart
                      timeline={result.timeline}
                      totalTime={Math.max(...result.timeline.map(s => s.end))}
                    />
                  </div>
